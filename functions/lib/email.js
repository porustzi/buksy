...
export function paymentConfirmedHtml({ orderId, amount, currency, paymentId, items, shippingCost, subtotal }) {
  const s = (v, len) => esc(sanitize(String(v || ''), len));
  const sc = Number(shippingCost) || 0;
  const st = Number(subtotal) || 0;
  const rows = [
    st > 0 ? `<tr><td style="padding:4px 0;color:#999">Товари</td><td style="padding:4px 0;text-align:right;font-weight:600">${st.toFixed(0)} ₴</td></tr>` : '',
    `<tr><td style="padding:4px 0;color:#999">Доставка</td><td style="padding:4px 0;text-align:right;font-weight:600">${sc > 0 ? sc.toFixed(0) + ' ₴' : 'БЕЗКОШТОВНО'}</td></tr>`,
    `<tr><td style="padding:6px 0 0;color:#fff;font-size:15px;font-weight:700">Разом</td><td style="padding:6px 0 0;text-align:right;font-size:18px;font-weight:700;color:#b10006">${Number(amount).toFixed(0)} ₴</td></tr>`,
  ].join('');
  let itemsHtml = '';
