      ...
      <div className="relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-6">
            <p className="text-white/50 font-body text-sm text-center sm:text-left max-w-md">{loc(nl, 'description')}</p>
            <div className="flex items-center gap-4">
              <a href={nl.telegramUrl} target="_blank" rel="noopener noreferrer" className="font-heading text-xl sm:text-2xl tracking-wider hover:text-blood transition-colors duration-300">{loc(nl, 'title')}</a>
              <a href={nl.telegramUrl} target="_blank" rel="noopener noreferrer" className="btn-primary inline-flex items-center gap-2 text-xs"><Send size={18} />{loc(nl, 'tg')}</a>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
          <div className="lg:col-span-1">
            ...
            <p className="text-white/60 font-body text-sm leading-relaxed mb-6">{loc(brand, 'description')}</p>
          ...
          <div>
            <h4 className="font-heading text-sm tracking-[0.3em] mb-6 text-white/40">{loc(nav, 'title')}</h4>
            <ul className="space-y-3">{(nav.links || []).map((l: any) => (<li key={l.href}><Link to={l.href} className="text-white/70 hover:text-blood transition-colors duration-300 font-body">{loc(l, 'name')}</Link></li>))}</ul>
          </div>
          <div>
            <h4 className="font-heading text-sm tracking-[0.3em] mb-6 text-white/40">{loc(sup, 'title')}</h4>
            <ul className="space-y-3">{(sup.links || []).map((l: any) => (<li key={l.href}><Link to={l.href} className="text-white/70 hover:text-blood transition-colors duration-300 font-body">{loc(l, 'name')}</Link></li>))}</ul>
          </div>
          <div>
            <h4 className="font-heading text-sm tracking-[0.3em] mb-6 text-white/40">{loc(leg, 'title')}</h4>
            <ul className="space-y-3">{(leg.links || []).map((l: any) => (<li key={l.href}><Link to={l.href} className="text-white/70 hover:text-blood transition-colors duration-300 font-body">{loc(l, 'name')}</Link></li>))}</ul>
          </div>
        </div>
      </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex flex-col items-center gap-4">
            ...
            <div className="text-center text-sm text-white/40 font-body">
              <p>&copy; {new Date().getFullYear()} BUKSY. {loc(copy, 'allRights')}</p>
            </div>
          </div>
        </div>
      </div>
