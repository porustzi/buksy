  ...
  return (
    <div className="overflow-hidden">
      ...
      <section className="relative h-screen">
          ...
          >
            ...
            >
              <Link to="/shop" className="btn-primary flex items-center gap-3">
                {h.hero?.shopNow || t('home.shopNow')}
                <ArrowRight size={18} />
              </Link>
              <Link to="/about" className="btn-secondary">
                {h.hero?.ourStory || t('home.ourStory')}
              </Link>
            </motion.div>
          </motion.div>
        </div>
        ...
        >
          <span className="text-white/40 text-xs tracking-[0.2em] font-body">{h.hero?.scroll || t('home.scroll')}</span>
      ...
      <section className="py-24 bg-noir">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            <div>
              <p className="section-subtitle mb-3">{h.featured?.tagline || t('home.selectedForYou')}</p>
              <h2 className="section-title">{h.featured?.title || t('home.featured')} <span className="text-blood">.</span></h2>
            </div>
            <Link to="/shop" className="group flex items-center gap-2 text-white/60 hover:text-white transition-colors duration-300">
              <span className="font-body text-sm tracking-wider">{h.featured?.viewAll || t('common.viewAll')}</span>
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid lg:grid-cols-2 gap-12 items-center">
          ...
          >
            <div className="aspect-[4/5] overflow-hidden">
              <img src={philImg} alt="BUKSY Editorial" className="w-full h-full object-cover" />
            </div>
          ...
          <motion.div
            ...
            className="lg:pl-8"
          >
            <p className="section-subtitle mb-3">{h.philosophy?.tagline || t('home.ourPhilosophy')}</p>
            <h2 className="section-title mb-8">{h.philosophy?.title1 || t('home.bornFrom')} <br />{h.philosophy?.title2 || t('home.darkness')}</h2>
            <div className="space-y-6 text-white/70 font-body leading-relaxed">
              <p>{h.philosophy?.text1 || t('home.philosophy1')}</p>
              <p>{h.philosophy?.text2 || t('home.philosophy2')}</p>
              <p>{h.philosophy?.text3 || t('home.philosophy3')}</p>
            </div>
            <Link to="/about" className="btn-primary inline-flex items-center gap-3 mt-10">
              {h.philosophy?.button || t('home.exploreStory')}
              <ArrowRight size={18} />
            </Link>
          </motion.div>
        </div>
      ...
      <section className="py-24 bg-noir">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            <p className="section-subtitle mb-3">{h.bestsellers?.tagline || t('home.mostWanted')}</p>
            <h2 className="section-title">{h.bestsellers?.title || t('home.bestsellers')} <span className="text-blood">.</span></h2>
          </motion.div>
      ...
      <section className="py-24 bg-ash">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div
            ...
            className="text-center mb-12"
          >
            <p className="section-subtitle mb-3">{h.collections?.tagline || t('home.shopByCategory')}</p>
            <h2 className="section-title">{h.collections?.title || t('home.collections')} <span className="text-blood">.</span></h2>
          </motion.div>
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
            {[
              ...
              <motion.div key={category.slug} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }}>
                <Link to={`/shop?category=${category.slug}`} className="group relative block aspect-[4/3] overflow-hidden bg-noir">
                  <img src={category.image} alt={t(category.nameKey)} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
