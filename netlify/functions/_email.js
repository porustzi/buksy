...
function orderConfirmationHtml({ orderId, items, total, shippingInfo, trackingNumber }) {
  ...
  var itemsRows = items.map(function (i) {
    var name = esc(i.product ? i.product.name : i.name || '');
    var size = esc(i.size || '');
  ...
  if (shippingInfo && shippingInfo.novaPoshtaBranch) {
    branchLine = '<tr><td style="padding:4px 0;color:#999;font-size:13px">Відділення НП</td><td style="padding:4px 0;text-align:right;font-size:13px;font-weight:600">№' + esc(String(shippingInfo.novaPoshtaBranch)) + '</td></tr>';
  }

  var firstName = esc(String(shippingInfo && shippingInfo.firstName || ''));
  var lastName = esc(String(shippingInfo && shippingInfo.lastName || ''));
  var address = esc(String(shippingInfo && shippingInfo.address || ''));
  var apartment = shippingInfo && shippingInfo.apartment ? ', ' + esc(String(shippingInfo.apartment)) : '';
  var city = esc(String(shippingInfo && shippingInfo.city || ''));
  var country = esc(String(shippingInfo && shippingInfo.country || ''));
  var postalCode = esc(String(shippingInfo && shippingInfo.postalCode || ''));
